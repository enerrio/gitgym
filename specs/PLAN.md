...
- [ ] Create `src/gitgym/exercise.py` with an `Exercise` dataclass: `name`, `topic`, `title`, `description`, `goal_summary`, `hints` (list of strings), `path` (path to exercise directory on disk)
...
- [ ] Implement `increment_hints_used(exercise_key: str)` — bumps the `hints_used` counter
- [ ] Implement `reset_exercise_progress(exercise_key: str)` — removes the exercise entry from progress
- [ ] Implement `reset_all_progress()` — deletes the progress file
- [ ] Implement `get_current_exercise() -> str | None` — returns the key of the exercise currently `in_progress` (or `None`)
- [ ] Write tests for each function using a temp file for progress.json

## Phase 4: Exercise Runner (setup & verify)

- [ ] Create `src/gitgym/runner.py` with `run_setup(exercise: Exercise) -> bool` that runs `setup.sh` via `subprocess`, passing the workspace exercise path as `$1`; returns True on success
- [ ] Implement `run_verify(exercise: Exercise) -> tuple[bool, str]` that runs `verify.sh`, captures stdout/stderr, returns `(success, output)`
- [ ] Ensure both functions handle missing scripts, non-executable scripts, and non-zero exit codes gracefully with clear error messages
...
- [ ] Implement `gitgym reset --all` — delete workspace and progress, print confirmation
...
- [ ] Create `exercises/01_basics/05_gitignore/` — setup creates a repo with files that should be ignored; verify checks `.gitignore` exists and specified files are excluded
