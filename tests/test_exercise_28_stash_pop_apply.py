"""End-to-end tests for exercises/08_stashing/02_stash_pop_apply."""
...
EXERCISE_DIR = EXERCISES_DIR / "08_stashing" / "02_stash_pop_apply"


@pytest.fixture
def work_dir(tmp_path):
    """Run setup.sh and return the exercise workspace directory."""
    d = tmp_path / "02_stash_pop_apply"
    d.mkdir()
    subprocess.run(
...
def test_exercise_toml_loads():
    ...
    assert ex.name == "stash_pop_apply"
...
def test_exercise_title():
    ex = load_exercise(EXERCISE_DIR)
    assert "stash" in ex.title.lower() or "apply" in ex.title.lower()
...
def test_setup_feature_txt_lacks_stashed_content(work_dir):
    """feature.txt should not have new_feature content before the stash is applied."""
    content = (work_dir / "feature.txt").read_text()
    assert "new_feature" not in content
...
def test_verify_fails_before_apply(work_dir):
    result = subprocess.run(
...
def test_verify_failure_mentions_apply(work_dir):
    result = subprocess.run(
        [str(EXERCISE_DIR / "verify.sh"), str(work_dir)],
        capture_output=True,
        text=True,
    ...
    assert "apply" in combined.lower() or "stash" in combined.lower()


def test_verify_fails_when_stash_popped_instead(work_dir):
    """Using git stash pop should cause verify to fail (stash list would be empty)."""
    subprocess.run(
        ["git", "stash", "pop"],
        cwd=work_dir,
        capture_output=True,
    ...
    assert result.returncode != 0
    combined = result.stdout + result.stderr
    assert "pop" in combined.lower() or "apply" in combined.lower()


# --- verify.sh: after goal is met ---


def test_verify_succeeds_after_stash_apply(work_dir):
    subprocess.run(
        ["git", "stash", "apply"],
        cwd=work_dir,
        capture_output=True,
...
def test_verify_success_output_is_congratulatory(work_dir):
    ...
    assert (
        ...
        or "apply" in combined.lower()
        or "stash" in combined.lower()
    )


def test_verify_stash_remains_after_apply(work_dir):
    subprocess.run(
        ["git", "stash", "apply"],
        cwd=work_dir,
        capture_output=True,
...
def test_verify_feature_txt_has_new_content_after_apply(work_dir):
    subprocess.run(
        ["git", "stash", "apply"],
        cwd=work_dir,
        capture_output=True,
